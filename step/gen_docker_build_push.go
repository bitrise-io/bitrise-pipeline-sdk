// Code generated by cmd/stepgen. DO NOT EDIT.
// Step: docker-build-push (1.0.1)

package step

import "time"

// DockerBuildPushPush enumerates the valid values for the push input.
type DockerBuildPushPush string

const (
	DockerBuildPushPushTrue  DockerBuildPushPush = "true"
	DockerBuildPushPushFalse DockerBuildPushPush = "false"
)

// DockerBuildPushUseBitriseCache enumerates the valid values for the use_bitrise_cache input.
type DockerBuildPushUseBitriseCache string

const (
	DockerBuildPushUseBitriseCacheTrue  DockerBuildPushUseBitriseCache = "true"
	DockerBuildPushUseBitriseCacheFalse DockerBuildPushUseBitriseCache = "false"
)

// DockerBuildPushBuildxHostNetwork enumerates the valid values for the buildx_host_network input.
type DockerBuildPushBuildxHostNetwork string

const (
	DockerBuildPushBuildxHostNetworkTrue  DockerBuildPushBuildxHostNetwork = "true"
	DockerBuildPushBuildxHostNetworkFalse DockerBuildPushBuildxHostNetwork = "false"
)

// DockerBuildPushVerbose enumerates the valid values for the verbose input.
type DockerBuildPushVerbose string

const (
	DockerBuildPushVerboseTrue  DockerBuildPushVerbose = "true"
	DockerBuildPushVerboseFalse DockerBuildPushVerbose = "false"
)

// DockerBuildPushBuilder builds a docker-build-push step with typed input methods.
type DockerBuildPushBuilder struct{ *Builder }

// DockerBuildPush creates a docker-build-push step builder (v1 by default).
// Pass an explicit major version to override the default:
//
//	step.DockerBuildPush("1")  // explicit default
//	step.DockerBuildPush("1")                  // older major
func DockerBuildPush(version ...string) *DockerBuildPushBuilder {
	v := "1"
	if len(version) > 0 && version[0] != "" {
		v = version[0]
	}
	return &DockerBuildPushBuilder{Builder: From("docker-build-push", v)}
}

// WithTags sets image tags.
func (b *DockerBuildPushBuilder) WithTags(value string) *DockerBuildPushBuilder {
	b.Builder.WithInput("tags", value)
	return b
}

// WithContext sets build context path.
func (b *DockerBuildPushBuilder) WithContext(value string) *DockerBuildPushBuilder {
	b.Builder.WithInput("context", value)
	return b
}

// WithFile sets dockerfile path.
func (b *DockerBuildPushBuilder) WithFile(value string) *DockerBuildPushBuilder {
	b.Builder.WithInput("file", value)
	return b
}

// WithPush sets push docker image.
func (b *DockerBuildPushBuilder) WithPush(value DockerBuildPushPush) *DockerBuildPushBuilder {
	b.Builder.WithInput("push", string(value))
	return b
}

// WithUseBitriseCache sets use Bitrise key-value cache.
func (b *DockerBuildPushBuilder) WithUseBitriseCache(value DockerBuildPushUseBitriseCache) *DockerBuildPushBuilder {
	b.Builder.WithInput("use_bitrise_cache", string(value))
	return b
}

// WithBuildArg sets build arguments.
func (b *DockerBuildPushBuilder) WithBuildArg(value string) *DockerBuildPushBuilder {
	b.Builder.WithInput("build_arg", value)
	return b
}

// WithCacheFrom sets cache from arguments.
func (b *DockerBuildPushBuilder) WithCacheFrom(value string) *DockerBuildPushBuilder {
	b.Builder.WithInput("cache_from", value)
	return b
}

// WithCacheTo sets cache to arguments.
func (b *DockerBuildPushBuilder) WithCacheTo(value string) *DockerBuildPushBuilder {
	b.Builder.WithInput("cache_to", value)
	return b
}

// WithExtraOptions sets extra options.
func (b *DockerBuildPushBuilder) WithExtraOptions(value string) *DockerBuildPushBuilder {
	b.Builder.WithInput("extra_options", value)
	return b
}

// WithBuildxHostNetwork sets enables to use the host network with the buildkit build container.
func (b *DockerBuildPushBuilder) WithBuildxHostNetwork(value DockerBuildPushBuildxHostNetwork) *DockerBuildPushBuilder {
	b.Builder.WithInput("buildx_host_network", string(value))
	return b
}

// WithVerbose sets verbose logging.
func (b *DockerBuildPushBuilder) WithVerbose(value DockerBuildPushVerbose) *DockerBuildPushBuilder {
	b.Builder.WithInput("verbose", string(value))
	return b
}

// WithVersion overrides the step version after construction. Prefer passing the version to the constructor instead.
func (b *DockerBuildPushBuilder) WithVersion(version string) *DockerBuildPushBuilder {
	b.Builder.WithVersion(version)
	return b
}

// WithInput sets a step input by raw key/value. Use this as an escape hatch for inputs not yet exposed as typed methods.
func (b *DockerBuildPushBuilder) WithInput(key, value string) *DockerBuildPushBuilder {
	b.Builder.WithInput(key, value)
	return b
}

// WithRunIf overrides the run_if expression for this step.
func (b *DockerBuildPushBuilder) WithRunIf(expr string) *DockerBuildPushBuilder {
	b.Builder.WithRunIf(expr)
	return b
}

// WithIsAlwaysRun configures whether this step runs even when a previous step failed.
func (b *DockerBuildPushBuilder) WithIsAlwaysRun(v bool) *DockerBuildPushBuilder {
	b.Builder.WithIsAlwaysRun(v)
	return b
}

// WithIsSkippable marks this step as skippable so a failure does not fail the build.
func (b *DockerBuildPushBuilder) WithIsSkippable(v bool) *DockerBuildPushBuilder {
	b.Builder.WithIsSkippable(v)
	return b
}

// WithTitle overrides the step title shown in the build log.
func (b *DockerBuildPushBuilder) WithTitle(title string) *DockerBuildPushBuilder {
	b.Builder.WithTitle(title)
	return b
}

// WithTimeout sets the maximum execution time in seconds. 0 disables the timeout.
func (b *DockerBuildPushBuilder) WithTimeout(seconds int) *DockerBuildPushBuilder {
	b.Builder.WithTimeout(seconds)
	return b
}

// WithNoOutputTimeout sets the maximum time the step may run without producing output.
func (b *DockerBuildPushBuilder) WithNoOutputTimeout(seconds int) *DockerBuildPushBuilder {
	b.Builder.WithNoOutputTimeout(seconds)
	return b
}

// WithTimeoutDuration sets the maximum execution time using a time.Duration.
func (b *DockerBuildPushBuilder) WithTimeoutDuration(d time.Duration) *DockerBuildPushBuilder {
	b.Builder.WithTimeoutDuration(d)
	return b
}

// WithNoOutputTimeoutDuration sets the no-output timeout using a time.Duration.
func (b *DockerBuildPushBuilder) WithNoOutputTimeoutDuration(d time.Duration) *DockerBuildPushBuilder {
	b.Builder.WithNoOutputTimeoutDuration(d)
	return b
}

// WithExecutionContainer pins this step to run inside the named container.
func (b *DockerBuildPushBuilder) WithExecutionContainer(containerID string) *DockerBuildPushBuilder {
	b.Builder.WithExecutionContainer(containerID)
	return b
}

// WithServiceContainers attaches one or more named service containers to this step.
func (b *DockerBuildPushBuilder) WithServiceContainers(containerIDs ...string) *DockerBuildPushBuilder {
	b.Builder.WithServiceContainers(containerIDs...)
	return b
}
