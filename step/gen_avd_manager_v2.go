// Code generated by cmd/stepgen. DO NOT EDIT.
// Step: avd-manager (2.0.2)

package step

import "time"

// AvdManagerV2Tag enumerates the valid values for the tag input.
type AvdManagerV2Tag string

const (
	AvdManagerV2TagGoogleApis               AvdManagerV2Tag = "google_apis"
	AvdManagerV2TagGoogleApisPs16k          AvdManagerV2Tag = "google_apis_ps16k"
	AvdManagerV2TagGoogleApisPlaystore      AvdManagerV2Tag = "google_apis_playstore"
	AvdManagerV2TagGoogleApisPlaystorePs16k AvdManagerV2Tag = "google_apis_playstore_ps16k"
	AvdManagerV2TagGoogleAtd                AvdManagerV2Tag = "google_atd"
	AvdManagerV2TagAospAtd                  AvdManagerV2Tag = "aosp_atd"
	AvdManagerV2TagAndroidWear              AvdManagerV2Tag = "android-wear"
	AvdManagerV2TagAndroidTv                AvdManagerV2Tag = "android-tv"
	AvdManagerV2TagDefault                  AvdManagerV2Tag = "default"
)

// AvdManagerV2Abi enumerates the valid values for the abi input.
type AvdManagerV2Abi string

const (
	AvdManagerV2AbiX86        AvdManagerV2Abi = "x86"
	AvdManagerV2AbiX8664      AvdManagerV2Abi = "x86_64"
	AvdManagerV2AbiArmeabiV7a AvdManagerV2Abi = "armeabi-v7a"
	AvdManagerV2AbiArm64V8a   AvdManagerV2Abi = "arm64-v8a"
	AvdManagerV2AbiMips       AvdManagerV2Abi = "mips"
)

// AvdManagerV2DisableAnimations enumerates the valid values for the disable_animations input.
type AvdManagerV2DisableAnimations string

const (
	AvdManagerV2DisableAnimationsYes AvdManagerV2DisableAnimations = "yes"
	AvdManagerV2DisableAnimationsNo  AvdManagerV2DisableAnimations = "no"
)

// AvdManagerV2EmulatorChannel enumerates the valid values for the emulator_channel input.
type AvdManagerV2EmulatorChannel string

const (
	AvdManagerV2EmulatorChannelNoUpdate AvdManagerV2EmulatorChannel = "no update"
	AvdManagerV2EmulatorChannel0        AvdManagerV2EmulatorChannel = "0"
	AvdManagerV2EmulatorChannel1        AvdManagerV2EmulatorChannel = "1"
	AvdManagerV2EmulatorChannel2        AvdManagerV2EmulatorChannel = "2"
	AvdManagerV2EmulatorChannel3        AvdManagerV2EmulatorChannel = "3"
)

// AvdManagerV2HeadlessMode enumerates the valid values for the headless_mode input.
type AvdManagerV2HeadlessMode string

const (
	AvdManagerV2HeadlessModeYes AvdManagerV2HeadlessMode = "yes"
	AvdManagerV2HeadlessModeNo  AvdManagerV2HeadlessMode = "no"
)

// AvdManagerV2Builder builds a avd-manager step with typed input methods.
//
// v1→v2: added `disable_animations`
type AvdManagerV2Builder struct{ *Builder }

// AvdManagerV2 creates a avd-manager step builder (v2 by default).
// Pass an explicit major version to override the default:
//
//	step.AvdManagerV2("2")  // explicit default
//	step.AvdManagerV2("1")                  // older major
//
// v1→v2: added `disable_animations`
func AvdManagerV2(version ...string) *AvdManagerV2Builder {
	v := "2"
	if len(version) > 0 && version[0] != "" {
		v = version[0]
	}
	return &AvdManagerV2Builder{Builder: From("avd-manager", v)}
}

// WithProfile sets device Profile ID.
func (b *AvdManagerV2Builder) WithProfile(value string) *AvdManagerV2Builder {
	b.Builder.WithInput("profile", value)
	return b
}

// WithApiLevel sets android API Level.
func (b *AvdManagerV2Builder) WithApiLevel(value string) *AvdManagerV2Builder {
	b.Builder.WithInput("api_level", value)
	return b
}

// WithTag sets oS Tag.
func (b *AvdManagerV2Builder) WithTag(value AvdManagerV2Tag) *AvdManagerV2Builder {
	b.Builder.WithInput("tag", string(value))
	return b
}

// WithAbi sets aBI.
func (b *AvdManagerV2Builder) WithAbi(value AvdManagerV2Abi) *AvdManagerV2Builder {
	b.Builder.WithInput("abi", string(value))
	return b
}

// WithDisableAnimations sets disable animations.
func (b *AvdManagerV2Builder) WithDisableAnimations(value AvdManagerV2DisableAnimations) *AvdManagerV2Builder {
	b.Builder.WithInput("disable_animations", string(value))
	return b
}

// WithEmulatorId sets iD.
func (b *AvdManagerV2Builder) WithEmulatorId(value string) *AvdManagerV2Builder {
	b.Builder.WithInput("emulator_id", value)
	return b
}

// WithCreateCommandFlags sets create AVD command flags.
func (b *AvdManagerV2Builder) WithCreateCommandFlags(value string) *AvdManagerV2Builder {
	b.Builder.WithInput("create_command_flags", value)
	return b
}

// WithStartCommandFlags sets start AVD command flags.
func (b *AvdManagerV2Builder) WithStartCommandFlags(value string) *AvdManagerV2Builder {
	b.Builder.WithInput("start_command_flags", value)
	return b
}

// WithEmulatorBuildNumber sets emulator build number.
func (b *AvdManagerV2Builder) WithEmulatorBuildNumber(value string) *AvdManagerV2Builder {
	b.Builder.WithInput("emulator_build_number", value)
	return b
}

// WithEmulatorChannel sets emulator channel.
func (b *AvdManagerV2Builder) WithEmulatorChannel(value AvdManagerV2EmulatorChannel) *AvdManagerV2Builder {
	b.Builder.WithInput("emulator_channel", string(value))
	return b
}

// WithHeadlessMode sets run the emulator in headless mode.
func (b *AvdManagerV2Builder) WithHeadlessMode(value AvdManagerV2HeadlessMode) *AvdManagerV2Builder {
	b.Builder.WithInput("headless_mode", string(value))
	return b
}

// WithVersion overrides the step version after construction. Prefer passing the version to the constructor instead.
func (b *AvdManagerV2Builder) WithVersion(version string) *AvdManagerV2Builder {
	b.Builder.WithVersion(version)
	return b
}

// WithInput sets a step input by raw key/value. Use this as an escape hatch for inputs not yet exposed as typed methods.
func (b *AvdManagerV2Builder) WithInput(key, value string) *AvdManagerV2Builder {
	b.Builder.WithInput(key, value)
	return b
}

// WithRunIf overrides the run_if expression for this step.
func (b *AvdManagerV2Builder) WithRunIf(expr string) *AvdManagerV2Builder {
	b.Builder.WithRunIf(expr)
	return b
}

// WithIsAlwaysRun configures whether this step runs even when a previous step failed.
func (b *AvdManagerV2Builder) WithIsAlwaysRun(v bool) *AvdManagerV2Builder {
	b.Builder.WithIsAlwaysRun(v)
	return b
}

// WithIsSkippable marks this step as skippable so a failure does not fail the build.
func (b *AvdManagerV2Builder) WithIsSkippable(v bool) *AvdManagerV2Builder {
	b.Builder.WithIsSkippable(v)
	return b
}

// WithTitle overrides the step title shown in the build log.
func (b *AvdManagerV2Builder) WithTitle(title string) *AvdManagerV2Builder {
	b.Builder.WithTitle(title)
	return b
}

// WithTimeout sets the maximum execution time in seconds. 0 disables the timeout.
func (b *AvdManagerV2Builder) WithTimeout(seconds int) *AvdManagerV2Builder {
	b.Builder.WithTimeout(seconds)
	return b
}

// WithNoOutputTimeout sets the maximum time the step may run without producing output.
func (b *AvdManagerV2Builder) WithNoOutputTimeout(seconds int) *AvdManagerV2Builder {
	b.Builder.WithNoOutputTimeout(seconds)
	return b
}

// WithTimeoutDuration sets the maximum execution time using a time.Duration.
func (b *AvdManagerV2Builder) WithTimeoutDuration(d time.Duration) *AvdManagerV2Builder {
	b.Builder.WithTimeoutDuration(d)
	return b
}

// WithNoOutputTimeoutDuration sets the no-output timeout using a time.Duration.
func (b *AvdManagerV2Builder) WithNoOutputTimeoutDuration(d time.Duration) *AvdManagerV2Builder {
	b.Builder.WithNoOutputTimeoutDuration(d)
	return b
}

// WithExecutionContainer pins this step to run inside the named container.
func (b *AvdManagerV2Builder) WithExecutionContainer(containerID string) *AvdManagerV2Builder {
	b.Builder.WithExecutionContainer(containerID)
	return b
}

// WithServiceContainers attaches one or more named service containers to this step.
func (b *AvdManagerV2Builder) WithServiceContainers(containerIDs ...string) *AvdManagerV2Builder {
	b.Builder.WithServiceContainers(containerIDs...)
	return b
}

// avdManagerV2Outputs holds the names of environment variables published
// by the avd-manager step (v2) at run time.
type avdManagerV2Outputs struct {
	// BitriseEmulatorSerial is the "Emulator serial" output env var.
	BitriseEmulatorSerial string
}

// AvdManagerV2Outputs provides typed access to the environment variable names
// that avd-manager (v2) exports after a successful run.
var AvdManagerV2Outputs = avdManagerV2Outputs{
	BitriseEmulatorSerial: "BITRISE_EMULATOR_SERIAL",
}
