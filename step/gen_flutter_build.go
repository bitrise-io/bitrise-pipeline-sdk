// Code generated by cmd/stepgen. DO NOT EDIT.
// Step: flutter-build (0.17.0)

package step

import "time"

// FlutterBuildPlatform enumerates the valid values for the platform input.
type FlutterBuildPlatform string

const (
	FlutterBuildPlatformBoth    FlutterBuildPlatform = "both"
	FlutterBuildPlatformIos     FlutterBuildPlatform = "ios"
	FlutterBuildPlatformAndroid FlutterBuildPlatform = "android"
)

// FlutterBuildIsDebugMode enumerates the valid values for the is_debug_mode input.
type FlutterBuildIsDebugMode string

const (
	FlutterBuildIsDebugModeTrue  FlutterBuildIsDebugMode = "true"
	FlutterBuildIsDebugModeFalse FlutterBuildIsDebugMode = "false"
)

// FlutterBuildCacheLevel enumerates the valid values for the cache_level input.
type FlutterBuildCacheLevel string

const (
	FlutterBuildCacheLevelAll  FlutterBuildCacheLevel = "all"
	FlutterBuildCacheLevelNone FlutterBuildCacheLevel = "none"
)

// FlutterBuildIosOutputType enumerates the valid values for the ios_output_type input.
type FlutterBuildIosOutputType string

const (
	FlutterBuildIosOutputTypeApp     FlutterBuildIosOutputType = "app"
	FlutterBuildIosOutputTypeArchive FlutterBuildIosOutputType = "archive"
)

// FlutterBuildAndroidOutputType enumerates the valid values for the android_output_type input.
type FlutterBuildAndroidOutputType string

const (
	FlutterBuildAndroidOutputTypeApk       FlutterBuildAndroidOutputType = "apk"
	FlutterBuildAndroidOutputTypeAppbundle FlutterBuildAndroidOutputType = "appbundle"
)

// FlutterBuildBuilder builds a flutter-build step with typed input methods.
type FlutterBuildBuilder struct{ *Builder }

// FlutterBuild creates a flutter-build step builder (v0 by default).
// Pass an explicit major version to override the default:
//
//	step.FlutterBuild("0")  // explicit default
//	step.FlutterBuild("1")                  // older major
func FlutterBuild(version ...string) *FlutterBuildBuilder {
	v := "0"
	if len(version) > 0 && version[0] != "" {
		v = version[0]
	}
	return &FlutterBuildBuilder{Builder: From("flutter-build", v)}
}

// WithProjectLocation sets project Location.
func (b *FlutterBuildBuilder) WithProjectLocation(value string) *FlutterBuildBuilder {
	b.Builder.WithInput("project_location", value)
	return b
}

// WithPlatform sets platform.
func (b *FlutterBuildBuilder) WithPlatform(value FlutterBuildPlatform) *FlutterBuildBuilder {
	b.Builder.WithInput("platform", string(value))
	return b
}

// WithAdditionalBuildParams sets additional params for flutter build.
func (b *FlutterBuildBuilder) WithAdditionalBuildParams(value string) *FlutterBuildBuilder {
	b.Builder.WithInput("additional_build_params", value)
	return b
}

// WithIsDebugMode sets debug mode?.
func (b *FlutterBuildBuilder) WithIsDebugMode(value FlutterBuildIsDebugMode) *FlutterBuildBuilder {
	b.Builder.WithInput("is_debug_mode", string(value))
	return b
}

// WithCacheLevel sets build cache.
func (b *FlutterBuildBuilder) WithCacheLevel(value FlutterBuildCacheLevel) *FlutterBuildBuilder {
	b.Builder.WithInput("cache_level", string(value))
	return b
}

// WithIosOutputType sets iOS output artifact type.
func (b *FlutterBuildBuilder) WithIosOutputType(value FlutterBuildIosOutputType) *FlutterBuildBuilder {
	b.Builder.WithInput("ios_output_type", string(value))
	return b
}

// WithIosCodesignIdentity sets codesign Identity.
func (b *FlutterBuildBuilder) WithIosCodesignIdentity(value string) *FlutterBuildBuilder {
	b.Builder.WithInput("ios_codesign_identity", value)
	return b
}

// WithIosAdditionalParams sets additional parameters.
func (b *FlutterBuildBuilder) WithIosAdditionalParams(value string) *FlutterBuildBuilder {
	b.Builder.WithInput("ios_additional_params", value)
	return b
}

// WithIosOutputPattern sets output pattern.
func (b *FlutterBuildBuilder) WithIosOutputPattern(value string) *FlutterBuildBuilder {
	b.Builder.WithInput("ios_output_pattern", value)
	return b
}

// WithAndroidOutputType sets android output artifact type.
func (b *FlutterBuildBuilder) WithAndroidOutputType(value FlutterBuildAndroidOutputType) *FlutterBuildBuilder {
	b.Builder.WithInput("android_output_type", string(value))
	return b
}

// WithAndroidAdditionalParams sets additional parameters.
func (b *FlutterBuildBuilder) WithAndroidAdditionalParams(value string) *FlutterBuildBuilder {
	b.Builder.WithInput("android_additional_params", value)
	return b
}

// WithAndroidOutputPattern sets output (.apk, .aab) pattern.
func (b *FlutterBuildBuilder) WithAndroidOutputPattern(value string) *FlutterBuildBuilder {
	b.Builder.WithInput("android_output_pattern", value)
	return b
}

// WithAndroidBundleOutputPattern sets [DEPRECATED] App bundle output pattern.
func (b *FlutterBuildBuilder) WithAndroidBundleOutputPattern(value string) *FlutterBuildBuilder {
	b.Builder.WithInput("android_bundle_output_pattern", value)
	return b
}

// WithVersion overrides the step version after construction. Prefer passing the version to the constructor instead.
func (b *FlutterBuildBuilder) WithVersion(version string) *FlutterBuildBuilder {
	b.Builder.WithVersion(version)
	return b
}

// WithInput sets a step input by raw key/value. Use this as an escape hatch for inputs not yet exposed as typed methods.
func (b *FlutterBuildBuilder) WithInput(key, value string) *FlutterBuildBuilder {
	b.Builder.WithInput(key, value)
	return b
}

// WithRunIf overrides the run_if expression for this step.
func (b *FlutterBuildBuilder) WithRunIf(expr string) *FlutterBuildBuilder {
	b.Builder.WithRunIf(expr)
	return b
}

// WithIsAlwaysRun configures whether this step runs even when a previous step failed.
func (b *FlutterBuildBuilder) WithIsAlwaysRun(v bool) *FlutterBuildBuilder {
	b.Builder.WithIsAlwaysRun(v)
	return b
}

// WithIsSkippable marks this step as skippable so a failure does not fail the build.
func (b *FlutterBuildBuilder) WithIsSkippable(v bool) *FlutterBuildBuilder {
	b.Builder.WithIsSkippable(v)
	return b
}

// WithTitle overrides the step title shown in the build log.
func (b *FlutterBuildBuilder) WithTitle(title string) *FlutterBuildBuilder {
	b.Builder.WithTitle(title)
	return b
}

// WithTimeout sets the maximum execution time in seconds. 0 disables the timeout.
func (b *FlutterBuildBuilder) WithTimeout(seconds int) *FlutterBuildBuilder {
	b.Builder.WithTimeout(seconds)
	return b
}

// WithNoOutputTimeout sets the maximum time the step may run without producing output.
func (b *FlutterBuildBuilder) WithNoOutputTimeout(seconds int) *FlutterBuildBuilder {
	b.Builder.WithNoOutputTimeout(seconds)
	return b
}

// WithTimeoutDuration sets the maximum execution time using a time.Duration.
func (b *FlutterBuildBuilder) WithTimeoutDuration(d time.Duration) *FlutterBuildBuilder {
	b.Builder.WithTimeoutDuration(d)
	return b
}

// WithNoOutputTimeoutDuration sets the no-output timeout using a time.Duration.
func (b *FlutterBuildBuilder) WithNoOutputTimeoutDuration(d time.Duration) *FlutterBuildBuilder {
	b.Builder.WithNoOutputTimeoutDuration(d)
	return b
}

// WithExecutionContainer pins this step to run inside the named container.
func (b *FlutterBuildBuilder) WithExecutionContainer(containerID string) *FlutterBuildBuilder {
	b.Builder.WithExecutionContainer(containerID)
	return b
}

// WithServiceContainers attaches one or more named service containers to this step.
func (b *FlutterBuildBuilder) WithServiceContainers(containerIDs ...string) *FlutterBuildBuilder {
	b.Builder.WithServiceContainers(containerIDs...)
	return b
}

// flutterBuildOutputs holds the names of environment variables published
// by the flutter-build step (v0) at run time.
type flutterBuildOutputs struct {
	// BitriseApkPath is the "The created .apk file's path" output env var.
	BitriseApkPath string
	// BitriseApkPathList is the "All created .apk file's path list" output env var.
	BitriseApkPathList string
	// BitriseAppDirPath is the "The generated `.app` directory" output env var.
	BitriseAppDirPath string
	// BitriseXcarchivePath is the "The generated `.xcarchive` directory" output env var.
	BitriseXcarchivePath string
	// BitriseXcarchiveZipPath is the "The generated `.xcarchive` directory compressed as a ZIP archive" output env var.
	BitriseXcarchiveZipPath string
	// BitriseAabPathList is the "List of the generated AAB file paths" output env var.
	BitriseAabPathList string
	// BitriseAabPath is the "Path of the generated AAB" output env var.
	BitriseAabPath string
}

// FlutterBuildOutputs provides typed access to the environment variable names
// that flutter-build (v0) exports after a successful run.
var FlutterBuildOutputs = flutterBuildOutputs{
	BitriseApkPath:          "BITRISE_APK_PATH",
	BitriseApkPathList:      "BITRISE_APK_PATH_LIST",
	BitriseAppDirPath:       "BITRISE_APP_DIR_PATH",
	BitriseXcarchivePath:    "BITRISE_XCARCHIVE_PATH",
	BitriseXcarchiveZipPath: "BITRISE_XCARCHIVE_ZIP_PATH",
	BitriseAabPathList:      "BITRISE_AAB_PATH_LIST",
	BitriseAabPath:          "BITRISE_AAB_PATH",
}
