// Code generated by cmd/stepgen. DO NOT EDIT.
// Step: brew-install (0.12.4)

package step

import "time"

// BrewInstallV0Upgrade enumerates the valid values for the upgrade input.
type BrewInstallV0Upgrade string

const (
	BrewInstallV0UpgradeYes BrewInstallV0Upgrade = "yes"
	BrewInstallV0UpgradeNo  BrewInstallV0Upgrade = "no"
)

// BrewInstallV0UseBrewfile enumerates the valid values for the use_brewfile input.
type BrewInstallV0UseBrewfile string

const (
	BrewInstallV0UseBrewfileYes BrewInstallV0UseBrewfile = "yes"
	BrewInstallV0UseBrewfileNo  BrewInstallV0UseBrewfile = "no"
)

// BrewInstallV0CacheEnabled enumerates the valid values for the cache_enabled input.
type BrewInstallV0CacheEnabled string

const (
	BrewInstallV0CacheEnabledYes BrewInstallV0CacheEnabled = "yes"
	BrewInstallV0CacheEnabledNo  BrewInstallV0CacheEnabled = "no"
)

// BrewInstallV0VerboseLog enumerates the valid values for the verbose_log input.
type BrewInstallV0VerboseLog string

const (
	BrewInstallV0VerboseLogYes BrewInstallV0VerboseLog = "yes"
	BrewInstallV0VerboseLogNo  BrewInstallV0VerboseLog = "no"
)

// BrewInstallV0Builder builds a brew-install step with typed input methods.
type BrewInstallV0Builder struct{ *Builder }

// BrewInstallV0 creates a brew-install step builder (v0 by default).
// Pass an explicit major version to override the default:
//
//	step.BrewInstallV0("0")  // explicit default
//	step.BrewInstallV0("1")                  // older major
func BrewInstallV0(version ...string) *BrewInstallV0Builder {
	v := "0"
	if len(version) > 0 && version[0] != "" {
		v = version[0]
	}
	return &BrewInstallV0Builder{Builder: From("brew-install", v)}
}

// WithPackages sets formulas to install.
func (b *BrewInstallV0Builder) WithPackages(value string) *BrewInstallV0Builder {
	b.Builder.WithInput("packages", value)
	return b
}

// WithUpgrade sets upgrade formula?.
func (b *BrewInstallV0Builder) WithUpgrade(value BrewInstallV0Upgrade) *BrewInstallV0Builder {
	b.Builder.WithInput("upgrade", string(value))
	return b
}

// WithUseBrewfile sets use a Brewfile to install packages?.
func (b *BrewInstallV0Builder) WithUseBrewfile(value BrewInstallV0UseBrewfile) *BrewInstallV0Builder {
	b.Builder.WithInput("use_brewfile", string(value))
	return b
}

// WithBrewfilePath sets path to the Brewfile.
func (b *BrewInstallV0Builder) WithBrewfilePath(value string) *BrewInstallV0Builder {
	b.Builder.WithInput("brewfile_path", value)
	return b
}

// WithOptions sets brew install/reinstall options.
func (b *BrewInstallV0Builder) WithOptions(value string) *BrewInstallV0Builder {
	b.Builder.WithInput("options", value)
	return b
}

// WithCacheEnabled sets enable cache?.
func (b *BrewInstallV0Builder) WithCacheEnabled(value BrewInstallV0CacheEnabled) *BrewInstallV0Builder {
	b.Builder.WithInput("cache_enabled", string(value))
	return b
}

// WithVerboseLog sets enable verbose logging?.
func (b *BrewInstallV0Builder) WithVerboseLog(value BrewInstallV0VerboseLog) *BrewInstallV0Builder {
	b.Builder.WithInput("verbose_log", string(value))
	return b
}

// WithVersion overrides the step version after construction. Prefer passing the version to the constructor instead.
func (b *BrewInstallV0Builder) WithVersion(version string) *BrewInstallV0Builder {
	b.Builder.WithVersion(version)
	return b
}

// WithInput sets a step input by raw key/value. Use this as an escape hatch for inputs not yet exposed as typed methods.
func (b *BrewInstallV0Builder) WithInput(key, value string) *BrewInstallV0Builder {
	b.Builder.WithInput(key, value)
	return b
}

// WithRunIf overrides the run_if expression for this step.
func (b *BrewInstallV0Builder) WithRunIf(expr string) *BrewInstallV0Builder {
	b.Builder.WithRunIf(expr)
	return b
}

// WithIsAlwaysRun configures whether this step runs even when a previous step failed.
func (b *BrewInstallV0Builder) WithIsAlwaysRun(v bool) *BrewInstallV0Builder {
	b.Builder.WithIsAlwaysRun(v)
	return b
}

// WithIsSkippable marks this step as skippable so a failure does not fail the build.
func (b *BrewInstallV0Builder) WithIsSkippable(v bool) *BrewInstallV0Builder {
	b.Builder.WithIsSkippable(v)
	return b
}

// WithTitle overrides the step title shown in the build log.
func (b *BrewInstallV0Builder) WithTitle(title string) *BrewInstallV0Builder {
	b.Builder.WithTitle(title)
	return b
}

// WithTimeout sets the maximum execution time in seconds. 0 disables the timeout.
func (b *BrewInstallV0Builder) WithTimeout(seconds int) *BrewInstallV0Builder {
	b.Builder.WithTimeout(seconds)
	return b
}

// WithNoOutputTimeout sets the maximum time the step may run without producing output.
func (b *BrewInstallV0Builder) WithNoOutputTimeout(seconds int) *BrewInstallV0Builder {
	b.Builder.WithNoOutputTimeout(seconds)
	return b
}

// WithTimeoutDuration sets the maximum execution time using a time.Duration.
func (b *BrewInstallV0Builder) WithTimeoutDuration(d time.Duration) *BrewInstallV0Builder {
	b.Builder.WithTimeoutDuration(d)
	return b
}

// WithNoOutputTimeoutDuration sets the no-output timeout using a time.Duration.
func (b *BrewInstallV0Builder) WithNoOutputTimeoutDuration(d time.Duration) *BrewInstallV0Builder {
	b.Builder.WithNoOutputTimeoutDuration(d)
	return b
}

// WithExecutionContainer pins this step to run inside the named container.
func (b *BrewInstallV0Builder) WithExecutionContainer(containerID string) *BrewInstallV0Builder {
	b.Builder.WithExecutionContainer(containerID)
	return b
}

// WithServiceContainers attaches one or more named service containers to this step.
func (b *BrewInstallV0Builder) WithServiceContainers(containerIDs ...string) *BrewInstallV0Builder {
	b.Builder.WithServiceContainers(containerIDs...)
	return b
}
