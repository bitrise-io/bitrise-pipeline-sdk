...
// valueOption represents a single allowed value for an input with value_options.
type valueOption struct {
	Value     string // raw value as written in step.yml, e.g. "app-store"
	ConstName string // full exported Go constant name, e.g. "XcodeArchiveV6DistributionMethodAppStore"
}
...
type inputDef struct {
	...
	MethodName   string
	Comment      string
	EnumTypeName string        // non-empty when the input has a fixed set of value_options
...
var genericBuilderMethods = []forwardingMethodDef{
	...
	{Name: "WithTimeoutDuration", Params: "(d time.Duration)", Call: "b.Builder.WithTimeoutDuration(d)",
		Comment: "WithTimeoutDuration sets the maximum execution time using a time.Duration."},
	{Name: "WithNoOutputTimeoutDuration", Params: "(d time.Duration)", Call: "b.Builder.WithNoOutputTimeoutDuration(d)",
		Comment: "WithNoOutputTimeoutDuration sets the no-output timeout using a time.Duration."},
...
// counterpart for emission in the gen_{id}_alias.go file.
type enumTypePair struct {
	Unversioned string // e.g. "XcodeArchiveDistributionMethod"
	Versioned   string // e.g. "XcodeArchiveV6DistributionMethod"
}
...
type aliasDef struct {
	...
	EnumTypes      []enumTypePair // enum type aliases to emit in the alias file
}
...
{{if .NeedsTimeImport}}
import "time"
{{end}}{{range .Inputs}}{{if .Options}}{{$enumType := .EnumTypeName}}
// {{.EnumTypeName}} enumerates the valid values for the {{.Key}} input.
...
{{end}}{{range .EnumTypes}}
// {{.Unversioned}} is a type alias for {{.Versioned}},
...
func generateMultiMajorStep(
	...
	// latestOutputCount and latestEnumTypes are populated while processing the
	// latest major version and used when building the aliasDef.
	...
	for _, major := range majorNums {
		...
		// parseStepYML built enum type names with the unversioned TypeName
		// (e.g. "MyStepMethod").  Re-apply the versioned TypeName so the
		// generated file uses "MyStepV2Method" and const names match.
		for i := range def.Inputs {
			inp := &def.Inputs[i]
			if inp.EnumTypeName == "" {
				continue
			}
			inp.EnumTypeName = def.TypeName + inp.MethodName
			for j := range inp.Options {
				inp.Options[j].ConstName = inp.EnumTypeName + toEnumConstSuffix(inp.Options[j].Value)
			}
		}
		...
		if major == latestMajor {
			...
			plainTypeName := toTypeName(stepID)
			for _, inp := range def.Inputs {
				if inp.EnumTypeName == "" {
					continue
				}
				// def.TypeName = "XcodeArchiveV6", plainTypeName = "XcodeArchive"
				// suffix = "DistributionMethod"
				suffix := strings.TrimPrefix(inp.EnumTypeName, def.TypeName)
				if suffix == "" {
					continue
				}
				latestEnumTypes = append(latestEnumTypes, enumTypePair{
					Unversioned: plainTypeName + suffix,
					Versioned:   inp.EnumTypeName,
				})
			}
		}

		// Skip writing the versioned file if it is already up-to-date.
		if upToDate {
			continue
		}
...
func parseStepYML(stepID, version string, data []byte) (*stepDef, error) {
	...
	for _, rawInput := range raw.Inputs {
		...
		if opts, ok := rawOpts.(map[interface{}]interface{}); ok {
			...
			// Build typed enum when the input has a fixed set of value_options.
			if rawValueOpts, ok := opts["value_options"]; ok {
				if valueOptsList, ok := rawValueOpts.([]interface{}); ok && len(valueOptsList) >= 2 {
					enumTypeName := def.TypeName + in.MethodName
					if options := buildValueOptions(enumTypeName, valueOptsList); options != nil {
						in.EnumTypeName = enumTypeName
						in.Options = options
					}
				}
			}
	...
	for _, m := range genericBuilderMethods {
		if !inputMethodSet[m.Name] {
			...
			if strings.Contains(m.Params, "time.Duration") {
				def.NeedsTimeImport = true
			}
		}
	}
...
// toEnumConstSuffix converts an arbitrary step.yml option value to a
// title-cased Go identifier suffix.  Non-alphanumeric characters act as word
// separators; the first letter of each word is uppercased and the rest are kept
// verbatim.  Returns an empty string when the value contains no letter or digit
// characters at all.
//
...
func toEnumConstSuffix(value string) string {
	var b strings.Builder
	startWord := true
	for _, r := range value {
		if unicode.IsLetter(r) || unicode.IsDigit(r) {
			if startWord && unicode.IsLetter(r) {
...
// buildValueOptions converts a raw value_options slice (from step.yml) into
// typed valueOption entries.  enumTypeName is prepended to each const suffix to
// form the full exported constant name.
//
// Returns nil when any value produces an empty or duplicate identifier — in
// those cases the caller should not emit an enum type for the input at all.
func buildValueOptions(enumTypeName string, rawValues []interface{}) []valueOption {
	opts := make([]valueOption, 0, len(rawValues))
	...
	for _, raw := range rawValues {
		...
		suffix := toEnumConstSuffix(strVal)
		if suffix == "" {
			return nil // value cannot become a valid identifier
		}
		constName := enumTypeName + suffix
		if seen[constName] {
			return nil // collision — skip this input entirely
		}
		seen[constName] = true
		opts = append(opts, valueOption{Value: strVal, ConstName: constName})
	}
	return opts
}
